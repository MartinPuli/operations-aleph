...
export function createModelRoutes(manager = new ModelManager()): Router {
  const routes = Router();
  routes.get('/api/settings/models', route(async (req, res) => res.json({ ...catalogResponse(), importAvailable: isLoopback(req), maxUploadBytes: MAX_MODEL_BYTES })));
  routes.get('/api/settings/models/downloads', (_req, res) => res.json({ jobs: downloadJobs() }));
  ...
  routes.post('/api/settings/models/:id/activate', route(async (req, res) => withModelManagement(async () => {
    ...
    res.json({ ok: true, ...catalogResponse() });
  })));
  return routes;
}
