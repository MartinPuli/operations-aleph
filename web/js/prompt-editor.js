...
function noteMarkup(note) {
  return note ? `<p class="note ${note.ok ? 'good' : 'bad'}" role="${note.ok ? 'status' : 'alert'}">${esc(note.text)}</p>` : '';
}
...
export function promptEditorMarkup(role) {
  ...
  const shell = `<section class="prompt-editor" id="${role}Prompts" aria-labelledby="${role}PromptsTitle" aria-busy="${promptEditor.loading || Boolean(promptEditor.busy)}"><div class="prompt-editor-heading"><div><h3 id="${role}PromptsTitle">${name} prompts</h3><p class="note">Edit the actual templates used by this installation. Saved changes apply to new requests.</p></div><button type="button" class="btn quiet" id="refreshPrompts"${promptEditor.loading || promptEditor.busy ? ' disabled' : ''}>Refresh prompts</button></div>`;
  if (!template) return `${shell}${promptEditor.loading ? '<div class="model-loading" role="status"><span class="sr-only">Loading prompt templates</span><i class="skeleton-line"></i><i class="skeleton-line short"></i></div>' : `<p class="note bad" role="alert">${esc(promptEditor.error || 'No prompt templates are available. Refresh to try again.')}</p>`}</section>`;
  const draft = draftFor(template);
  const busy = Boolean(promptEditor.busy);
  return `${shell}
    ${promptEditor.error ? `<p class="note bad" role="alert">${esc(promptEditor.error)} Your drafts are kept.</p>` : ''}
    <form id="promptEditorForm" novalidate>
      ...
      <p class="note prompt-description">${esc(template.description)}</p>
      <div class="field prompt-text-field"><label for="promptTemplateText">Template text</label><textarea id="promptTemplateText" data-no-restore spellcheck="false" autocapitalize="off" autocomplete="off" maxlength="32768" aria-describedby="promptTextHelp promptValidation" aria-invalid="${draft.errors.length > 0}"${busy ? ' readonly' : ''}>${esc(draft.text)}</textarea><p class="note" id="promptTextHelp">${template.tokens.length ? 'Warden fills the variables below with request data. Keep required variables exactly as written.' : 'This template is used as written; it has no replaceable variables.'} Up to 32,768 characters. Ctrl+Enter or ⌘+Enter saves.</p><div id="promptValidation" role="alert">${errorMarkup(draft.errors)}</div></div>
      <div class="prompt-variables"><h4>Variables</h4><p class="note">Select a variable to insert it at the text cursor.</p>${template.tokens.length ? `<ul>${template.tokens.map((token) => `<li><button type="button" class="btn quiet prompt-token" data-prompt-token="${esc(token.name)}"${busy ? ' disabled' : ''}><code>${esc(tokenText(token.name))}</code><span class="sr-only">Insert variable</span></button><div><span class="prompt-variable-kind">${token.required || template.requiredTokens?.includes(token.name) ? 'Required' : 'Optional'}</span><p>${esc(token.description)}</p></div></li>`).join('')}</ul>` : '<p class="note">This template has no variables.</p>'}</div>
      <details class="prompt-reference"><summary>Required response format · read only</summary><pre>${esc(template.outputContract)}</pre><p class="note">Saving a prompt does not change this response contract or certify its policy accuracy.</p></details>
...
function updateDraftControls(template, draft) {
  ...
  if ($('promptDraftStatus')) { $('promptDraftStatus').textContent = draftStatus(draft); $('promptDraftStatus').className = `note${dirty(draft) ? ' warn' : ''}`; }
