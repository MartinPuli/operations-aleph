...
export function documentComposer() {
  ...
  return `<div class="document-composer" id="documentDropzone" aria-busy="${reading}">
    <div class="document-attach-row"><input id="documentFiles" class="sr-only" type="file" accept="${esc(accept)}" multiple aria-label="Attach documents" aria-describedby="documentLimits"${state.sending || reading ? ' disabled' : ''}><button type="button" class="btn quiet" id="attachDocuments"${state.sending || reading ? ' disabled' : ''}><svg class="ui-icon" viewBox="0 0 20 20" fill="none" aria-hidden="true"><path d="m7 11 5-5a2 2 0 0 1 3 3l-6 6a4 4 0 0 1-6-6l6-6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>Attach files</button><span class="note" id="documentLimits">or drop here · ${limits.files} files · ${fileSize(limits.fileBytes)} each</span></div>
    <p class="document-formats">PDF, Word, text, scans and images · ${fileSize(limits.totalBytes)} total</p>
    ${selected.length ? `<ul class="document-selection" aria-label="Files ready to check">${selected.map((file) => `<li><span class="document-extension" aria-hidden="true">${esc(file.name.split('.').pop().slice(0, 5).toUpperCase())}</span><div><b>${esc(file.name)}</b><span class="note">${fileSize(file.bytes)} · ${state.sending ? 'Checking with your prompt' : 'Ready to check'}</span></div><button type="button" class="btn quiet" data-remove-document="${file.id}" aria-label="Remove ${esc(file.name)}"${state.sending || reading ? ' disabled' : ''}><svg class="ui-icon" viewBox="0 0 20 20" fill="none" aria-hidden="true"><path d="m5 5 10 10M15 5 5 15" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button></li>`).join('')}</ul>` : ''}
