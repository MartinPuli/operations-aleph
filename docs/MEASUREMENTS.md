...
| `WARDEN_ADJUDICATOR_FORM=dynaguard-native` / bench `dynaguard-native` | Added 2026-09-05. DynaGuard's question exactly as its model card gives it — "provide the final answer directly", a numbered policy, `<answer>PASS</answer>` as free text, no grammar — against the grammar-constrained JSON the seat ships with. The shipped form was a deliberate first cut and its own note named this as the next measurement. |
| `WARDEN_POLICY_SCREEN=1` (eval, DynaGuard seats only) | Added 2026-09-05. One call over the whole applicable policy before any per-rule call: PASS skips the per-rule calls, FAIL runs them, an unattributed FAIL escalates. Honest requests cost one call instead of four and every rule is read rather than the top three. The base model answered one broad question worse than four narrow ones; the fine-tune was trained on multi-rule policies. |
| `dynaguard-8b` seat / `adjudicator-dynaguard-8b` | Added 2026-09-05, unmeasured. Same training as the default on twice the weights, DynaBench F1 72.5 against 68.2. `adjudicator-qwen3-4b` is the size control beside it. See `docs/INFERENCE-AND-MODELS.md`, "Which model, decided". |
