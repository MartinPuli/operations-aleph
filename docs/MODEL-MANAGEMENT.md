...
does not resume a custom download. Built-in desktop downloads retain their
existing resumable downloader.
...
| --- | --- |
| `GET /` | `{models, selections, overrides, inForce, importAvailable, maxUploadBytes}` |
...
| `GET /downloads` | `{jobs:[{id,name,state,received,total,modelId,error}]}` |
| `DELETE /downloads/:id` | Cancels the job; partial bytes are removed |
...
already on disk are checked before activation. Choosing an absent built-in
model is a download request, and the currently loaded judge keeps serving until
the desktop downloader finishes and restarts. Its identity, declared prompt
